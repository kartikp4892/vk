...
function! s:ConvertToCodes( codeline )
	" keywords that don't affect indent: module endmodule package endpackage interface endinterface
        let delims = substitute(a:codeline, "\\<\\(\\%(initial\\|always\\|always_comb\\|always_ff\\|always_latch\\|final\\|begin\\|if\\|for\\|foreach\\|do\\|while\\|forever\\|repeat\\|case\\|fork\\(\\_s*;\\)\\@!\\|ifdef\\|else\\|end\\|endif\\|endcase\\|join\\|join_any\\|join_none\\|class\\|config\\|clocking\\|function\\|task\\|specify\\|covergroup\\|pure\\|assume\\|assert\\|cover\\|endclass\\|endconfig\\|endclocking\\|endfunction\\|endtask\\|endspecify\\|endgroup\\|\\(assert\\s\\+\\)\\@<!property\\|endproperty\\|sequence\\|checker\\|endsequence\\|endchecker\\|ovm_\\w\\+begin\\|ovm_\\w\\+end\\)\\>\\)\\@!\\k\\+", "", "g")
	...
	let delims = substitute(delims, "\\(`\\<endif\\>\\)", "e", "g")
        let delims = substitute(delims, "\\(`\\<ovm_\\w\\+begin\\>\\)", "b", "g")
        let delims = substitute(delims, "\\(`\\<ovm_\\w\\+end\\>\\)", "e", "g")
	let delims = substitute(delims, "\\(`\\<else\\>\\)", "eb", "g")
	let delims = substitute(delims, "\\<\\(begin\\|case\\|fork\\(\\_s*;\\)\\@!\\)\\>", "b", "g")
	let delims = substitute(delims, "\\<\\(end\\|endcase\\|join\\|join_any\\|join_none\\)\\>", "e", "g")
	let delims = substitute(delims, "\\<\\(class\\|config\\|clocking\\|function\\|task\\|specify\\|covergroup\\|\\(assert\\s\\+\\)\\@<!property\\|sequence\\|checker\\)\\>", "f", "g")
