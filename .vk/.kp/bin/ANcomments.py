...
# ||   def process (self):
# ||     """def: process"""
# ||     os.system('vim -N -u NONE -Resn +"redir > /dev/stdout" +"BufdoANcomments" +"redir END" +"qa!" {infiles}'.format(infiles=' '.join(self.infiles)))
# ||     #os.system('vim -N -u NONE -Resn -s $KP_VIM_HOME/sv/commands.vim +"BufdoANcomments" +"qa!" {infiles}'.format(infiles=' '.join(self.infiles)))
...
if __name__ == "__main__":
  ...
  if out != '':
    ...
    os.system('gvim -n +"nmap <F5> :SearchInBuffer \'__TBD__\'<CR>" +"source $KP_VIM_HOME/source_scripts.vim" {files}'.format(files=' '.join(tbd_files)))
