...
function GetInFiles {
  ...
  do
    if [[ -d "${arg}" ]] ; then
      input_files+=( "$(find ${arg} -regextype sed -regex '.*\.sv[hi]\?$')" )
    else
      if [[ ${arg} =~ .*\.sv[ih]? ]] ; then
...
# Without Multithread
#gvim -N -u NONE -Rnc  "py import sys" +"py sys.argv = ['-d', '${out_dir}']" +"pyfile $KP_VIM_HOME/python_lib/vim/ctags/main.py" ${input_files[@]}
